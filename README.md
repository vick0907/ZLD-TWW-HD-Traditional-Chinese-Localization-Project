...
選單字型維持不變。`zh-TW` 分支已採用；正式下載版 `tw-v1.0.7` 尚未包含這項調整。
...
| 項目 | 內容 |
|---|---|
| 文字 | 5,040 句全數轉繁，4,279 句有變動 |
| 字型 | `CKingMsg` 4,390 → 5,389 字，漢字採 500 字重；`CKingMain`/`MainL` 569 → 748 字 |
...
`text/readability_pass.json` 收錄 78 個精確替換，涵蓋 54 則訊息；兩組獨立
...
建置流程：解包 → 轉繁 → 套用三輪人工補丁 → 重建英中對照與 QA →
